...
#creation of tables:
if(Db.request("SELECT COUNT(1) FROM sqlite_master") == 0):
...
if(Db.request("SELECT COUNT(1) FROM Courses") == 0):
	Db.write("INSERT INTO Courses VALUES (1, 'python', '21.07.21', '21.08.21'), (2, 'java', '13.07.21', '16.08.21');")
if(Db.request("SELECT COUNT(1) FROM Students") == 0):
...
if(Db.request("SELECT COUNT(1) FROM Student_courses") == 0):
	Db.write("INSERT INTO Student_courses VALUES (1, 1),(2, 1),(3, 1),(4, 2);")

#request

#1. Всех студентов старше 30 лет.
print(Db.request("SELECT name FROM Students WHERE age > 30;"))

#2. Всех студентов, которые проходят курс по python.
print(Db.request("SELECT name FROM Students WHERE id IN (SELECT student_id FROM Student_courses WHERE course_id IN (SELECT id FROM Courses WHERE name = 'python'));"))

#3. Всех студентов, которые проходят курс по python и из Spb.
print(Db.request("SELECT name FROM Students WHERE id IN (SELECT student_id FROM Student_courses WHERE course_id IN (SELECT id FROM Courses WHERE name = 'python')) AND city = 'Spb';"))
